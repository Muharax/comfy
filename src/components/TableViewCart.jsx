...
const TableViewCart = () => {
    ...
    const { handleClickDelte,
        handleClickDelteAll } = useContext(ConfirmModalContext);
        ...
        */
        cart.length && !isBtnEmptyCart ? <div className="table-box">
                        ...
                        return (
                            <tr key={key}>
                                ...
                                <td className='text-center fw-bold'>
                                    {val.pSalePrice ? <><span>${netPriceAfterSale}</span><span className="lineTrough">${val.pPrice}</span></> : <span>${val.pPrice}</span>}
                                </td>
                                ...
                                <td className='text-center'>
                                    <div className='product-counter d-flex align-items-center justify-content-center gap-2'>
                                        ...
                                        */}
                                        <button className="counter-btn" disabled={val.pQty === 1 ? true : ""} onClick={() => handleDecreaseCounter(val.pId)}>
                                            <FaSquareMinus />
                                        </button>
                                        <span>{val.pQty}</span>
                                        {/* 
                                        ...
                                        */}
                                        <button className="counter-btn" disabled={val.pQty >= Number(val.pInStock) ? true : ""} onClick={() => handleIncreaseCounter(val.pId)}>
                                            <FaSquarePlus />
                                        </button>
                                    </div>
                                </td>
                    ...
                    <tr>
                        <td colSpan="7">
                            <div className='my-flex-div row justify-content-between align-items-center mx-0 py-2'>
                                <div className="col-lg-4 col-sm-4"><Link to="/shop" className='continue-shop text-decoration-none d-flex align-items-center gap-2 bg-dark text-white'><FaRegCircleLeft /> Continue Shopping</Link></div>
                                <div className="col-lg-5 col-sm-4"><button className='empty-btn text-decoration-none d-flex align-items-center gap-2 bg-dark text-white' onClick={handleClickDelteAll}><FaRegTrashCan /> Empty Shopping Cart</button></div>
                                <div className="col-lg-3 col-sm-4 text-lg-center text-md-start">TOTAL PRICE:  <span className='fw-bold' style={{ color: "#c7ab62" }}>${fixedTotal}</span></div>
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <div className='text-center'>
                <Link to="/checkout/information" className='table-checkout d-inline-block text-center text-decoration-none bg-dark text-white px-3 py-2 mt-4 w-50'>Checkout</Link>
            </div>
        </div>
            :
            <>
